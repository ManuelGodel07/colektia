...
const DashBoss = () => {
  ...
  const [chartData, setChartData] = useState([]); // NUEVO ESTADO PARA LA GRAFICA
  ...
  const getAllDocuments = async (docu) => {
    ...
    const counts = {}; // objeto para contar por mes
    ...
    // Transformar a arreglo para la gráfica
    const chart = Object.keys(counts)
      .sort()
      .map((key) => {
  ...
  // Calcular promedio y lista filtrada cuando cambia el mes
  useEffect(() => {
