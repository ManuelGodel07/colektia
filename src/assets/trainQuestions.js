export const trainSet = [
    {
        questionText: "¿Consideras que la capacitación fue clara y bien estructurada?",
        name: "1",
        options: [
            { choice: "a) Si", radioValue: "q1-a", selected: false },
            { choice: "b) Mas o menos", radioValue: "q1-a", selected: false },
            { choice: "c) No", radioValue: "q1-a", selected: false }
        ]
    },
    {
    questionText: "¿La capacitación cubrió todas las habilidades y conocimientos necesarios para tu rol?",
    name: "2",
    options: [
        { choice: "a) Si", radioValue: "q2-a", selected: false },
        { choice: "b) Mas o menos", radioValue: "q2-b", selected: false },
        { choice: "c) No", radioValue: "q2-c", selected: false } ]
    },
    {
    questionText: "¿Los instructores o facilitadores fueron efectivos y respondieron tus dudas adecuadamente?",
    name: "3",
    options: [
        { choice: "a) Si", radioValue: "q3-a", selected: false },
        { choice: "b) Mas o menos", radioValue: "q3-b", selected: false },
        { choice: "c) No", radioValue: "q3-c", selected: false }]
    },
    {
    questionText: "¿Recibiste suficiente tiempo y apoyo para practicar lo aprendido durante la capacitación?",
    name: "4",
    options: [
        { choice: "a) Si", radioValue: "q4-a", selected: false },
        { choice: "b) No", radioValue: "q4-b", selected: false }
    ]
    },
    {
        questionText: "¿Qué tan fácil te fue aplicar tus conocimientos y habilidades a la capacitación",
        name: "5",
        options: [
            { choice: "a) Buena", radioValue: "q5-a", selected: false },
            { choice: "b) Regular", radioValue: "q5-b", selected: false },
            { choice: "c) Mala", radioValue: "q5-c", selected: false }]
    },
    {
        questionText: "¿Consideras que la duración de la capacitación fue adecuada para cubrir todos los temas?",
        name: "6",
        options: [
            { choice: "a) Si", radioValue: "q6-a", selected: false },
            { choice: "a) No", radioValue: "q6-b", selected: false }]
    },
    {
        questionText: "¿Te sientes list para realizar tus tareas con éxito después de la capacitación?",
        name: "7",
        options: [
            { choice: "a) Si", radioValue: "q7-a", selected: false },
            { choice: "b) No", radioValue: "q7-b", selected: false } ]
    }
];